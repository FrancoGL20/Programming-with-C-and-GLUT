...
void carga_texturas(){
    cargaTextura_raw(1000, 500, "PTierra.data", &texturas[0], 0, GL_TRUE);
    cargaTextura_raw(127, 127, "madera.data", &texturas[1], 1, GL_FALSE);
    for (int i = 0; i < 64; i++) // Textura procedural espacio estrellado
        for (int j = 0; j < 64; j++)        {
    ...
    for (int i = 0; i < 32; i++) // Textura procedural Tux
        for (int j = 0; j < 32; j++){
